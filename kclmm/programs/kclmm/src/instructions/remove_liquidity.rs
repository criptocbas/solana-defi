...
pub fn handle_remove_liquidity(
    ...
    let lower_idx = math::tick_index_in_array(
        tick_lower, ta_lower.start_tick_index, pool.tick_spacing,
    ...
    let upper_idx = math::tick_index_in_array(
        tick_upper, ta_upper.start_tick_index, pool.tick_spacing,
    ).ok_or(error!(KclmmError::TickNotInArray))?;
    ...
    // --- Update tick states ---
    {
        ...
        if tick_data.liquidity_gross == 0 {
            tick_data.fee_growth_outside_a = 0;
            tick_data.fee_growth_outside_b = 0;
            tick_data.liquidity_net = 0;
            math::clear_bit(&mut ta.initialized_bitmap, lower_idx);
        }
    }

    {
        ...
        if tick_data.liquidity_gross == 0 {
            ...
            tick_data.fee_growth_outside_b = 0;
            tick_data.liquidity_net = 0;
            math::clear_bit(&mut ta.initialized_bitmap, upper_idx);
        }
    }
    ...
    // --- Compute token amounts (round DOWN for withdrawals) ---
    ...
    // --- Slippage check ---
    require!(amount_a >= amount_a_min, KclmmError::AmountABelowMin);
    require!(amount_b >= amount_b_min, KclmmError::AmountBBelowMin);
